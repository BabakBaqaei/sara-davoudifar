"""زمینه‌ی قرمز — نسخه‌ی نهایی، با برشِ کادرِ خالی.

اسکرین‌ریکوردرِ بابک ایراد را نشان داد و اندازه‌گیری جایش را دقیق کرد:
سارا در فریمِ ۱۶۳ منبع (**۶.۷۹ ثانیه**) کامل از کادر بیرون می‌رود، ولی
منبع ۱.۲۱ ثانیه فریمِ خالی بعدش دارد و نسخه‌ی قبل همه‌اش را برمی‌داشت.
نتیجه: **۱۴٪ از لوپ کادرِ قرمزِ خالی** بود. روی دیوارِ کرِم این سکوت
طبیعی به‌نظر می‌رسید؛ روی قرمزِ تیره مثل خرابیِ ویدیو دیده می‌شود.

حالا آخرین بخش در ۷.۰۰ ثانیه تمام می‌شود: ۰.۲۱ ثانیه کادرِ خالی می‌مانَد
که فقط درزِ لوپ را می‌بندد.

**فرضیه‌ای که رد شد:** اول فکر کردم لبه‌ی مات «می‌جوشد» و میانه‌ی زمانی
گذاشتم. اندازه‌گیری بعدی نشان داد ماتِ خام پایدار است (۰.۲۱٪ سطح) و آن
عددِ اولیه نویزِ فشرده‌سازی بود. میانه برداشته شد.

آلفاها در `alpha3/` کش می‌شوند تا اجرای دوباره rembg نخواهد.
"""
...
SEGS = [(0.10,1.90,1.00),(1.90,3.30,1.00),(3.30,5.60,0.62),(5.60,6.55,1.00),(6.55,7.00,1.00)]
...
lit=np.array([0x64,0x02,0x16],np.float32); mid=np.array([0x4a,0x02,0x0c],np.float32); deep=np.array([0x2a,0x01,0x05],np.float32)
t_=np.clip(d,0,1)[...,None]
bg=np.where(t_<0.46, lit+(mid-lit)*(t_/0.46), mid+(deep-mid)*((t_-0.46)/0.54)).astype(np.float32)

os.makedirs("alpha3", exist_ok=True)
sess = new_session("isnet-general-use")
def raw_alpha(i):
    cp = "alpha3/%04d.png" % i
    if os.path.exists(cp):
        return np.asarray(Image.open(cp).convert("L")).astype(np.float32)
    im = Image.open("src3/"+FR[i]).convert("RGB")
    a = np.asarray(remove(im, session=sess).split()[-1]).astype(np.float32)
    a = np.clip((a-165)/(255-165),0,1)*255
    small = a[::4,::4] > 40
    ...
    Image.fromarray(a.astype(np.uint8)).save(cp)
    return a

print("computing mattes...")
for i in range(N):
    raw_alpha(i)
    if i % 40 == 0: print("   %d/%d" % (i, N), flush=True)

def smooth_alpha(i):
    """یک پیکسل تو و نرم. میانه‌ی زمانی هم آزموده شد و **بی‌اثر** بود:
    نوسانِ خودِ مات در بازه‌ی مکث فقط ۰.۲۱٪ سطحِ سوژه است (حدود ۰.۱۵px
    جابه‌جاییِ لبه) و آنچه اول به‌عنوان «جوشش» اندازه گرفته بودم، نویزِ
    فشرده‌سازیِ خروجی بود نه ماتِ ناپایدار."""
    a = raw_alpha(i)
...
def comp(i):
    ...
    im = Image.open("src3/"+FR[i]).convert("RGB")
    al = smooth_alpha(i)
    out = np.asarray(im).astype(np.float32)*al + bg*(1-al)
    cache[i]=Image.fromarray(np.clip(out,0,255).astype(np.uint8))
    return cache[i]

timeline=[]
for s0,s1,sp in SEGS:
    n=int(round((s1-s0)/sp*OUT_FPS)); timeline+=[s0+(k/n)*(s1-s0) for k in range(n)]
...
for i,ts in enumerate(timeline):
    im = comp(int(round(ts*SRC_FPS)))
    tt=i/OUT_FPS
    if tt < BLUR_END:
        r=BLUR_MAX*(1-tt/BLUR_END)**1.8
    ...
    if i%50==0: print("  %d/%d"%(i,len(timeline)), flush=True)
