...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      ...
      >
        <div
          className="bg-neutral-950 border border-white/10 rounded-xl max-w-md w-full overflow-hidden"
        ...
        >
          <div className="relative aspect-square">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-2 left-2 w-8 h-8 bg-black/60 rounded-full text-white flex items-center justify-center hover:bg-black/80 transition-all cursor-pointer"
              aria-label="بستن"
            >
              <X size={16} />
            </button>
          ...
          {(selectedImage.title || selectedImage.description) && (
            <div className="p-6">
              ...
              {selectedImage.description && (
                <p className="text-sm text-neutral-400 leading-loose">{selectedImage.description}</p>
              )}
            </div>
          )}
        </div>
  ...
  if (!isMounted) {
    return (
      <div
        className="rounded-lg animate-pulse flex items-center justify-center bg-white/5"
        style={{ width: containerSize, height: containerSize }}
      >
        <div className="text-neutral-500 text-sm">در حال بارگذاری…</div>
      </div>
    );
  }
  ...
  if (!images.length) {
    return (
      <div
        ...
        style={{ width: containerSize, height: containerSize }}
      >
        <div className="text-neutral-500 text-center text-sm">تصویری وجود ندارد</div>
      </div>
    );
  }
