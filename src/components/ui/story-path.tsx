...
/**
 * کفِ هر کارت، **اندازه‌گیری‌شده در مرورگر**. ارتفاع‌ها یکی نیستند —
 * «سقوط» و «انتخاب» متنِ کوتاه‌تری دارند — و همین بود که خط را خراب
 * می‌کرد: با افستِ ثابت، سرِ خط یا از کارت می‌زد بیرون یا آن‌قدر تو
 * می‌رفت که از کنارِ کارتِ بعدی سر درمی‌آورد.
 *
 * اگر متنِ کارت‌ها عوض شد، این‌ها را دوباره اندازه بگیر.
 */
const BOTTOMS = [319, 721, 1119, 1449];

/* خط از ۲۰ پیکسل داخلِ کفِ کارتِ قبلی درمی‌آید و ۲۰ پیکسل داخلِ سرِ
   کارتِ بعدی می‌رود: دو سرش پشتِ کارت پنهان‌اند و آنچه دیده می‌شود
   دقیقاً دهانه‌ی بینِ دو کارت است، نه بیشتر. */
const LINK = TOPS.slice(0, -1).map((t, i) => {
  const x0 = CX[i % 2];
  const x1 = CX[(i + 1) % 2];
  const y0 = BOTTOMS[i] - 20;
  const y1 = TOPS[i + 1] + 20;
  const pull = (y1 - y0) * 0.55;
  return `M ${x0} ${y0} C ${x0} ${y0 + pull}, ${x1} ${y1 - pull}, ${x1} ${y1}`;
});
