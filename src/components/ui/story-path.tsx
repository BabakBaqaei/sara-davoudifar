...
const LINK = TOPS.slice(0, -1).map((t, i) => {
  const x0 = CX[i % 2];
  const x1 = CX[(i + 1) % 2];
  /* ۱۵۰ و نه ۲۷۰: کارتِ «سقوط» حدود ۲۵۰ پیکسل بلند است و نه ۳۲۰، پس
     ۲۷۰ از تهِ آن می‌زد بیرون و سرِ خطِ آخر وسطِ فضای خالی پیدا می‌شد —
     برخلافِ سه خطِ قبلی که از پشتِ کارت درمی‌آمدند. ۱۵۰ داخلِ هر پنج
     کارت می‌افتد، کوتاه یا بلند. */
  const y0 = t + 150;
